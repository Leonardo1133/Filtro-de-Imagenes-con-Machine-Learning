...
%im = rgb2gray(im);                                 % pasa la imagen a gris    
im_norm   = im2double(im);                          % normaliza la imagen a valores entre 0 y 1
im_ruido = imnoise(im_norm,'gaussian', 0, 0.005);    % im_map_saltPepper = imnoise(im_map_gaussian,'salt & pepper',0.1);  imnoise(im_map,'gaussian',media,varianza o densidad)  
